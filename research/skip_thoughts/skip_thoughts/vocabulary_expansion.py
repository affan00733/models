...
def _load_skip_thoughts_embeddings(embedding_matrix_file):
...
def _load_vocabulary(filename):
    ...
    with tf.gfile.GFile(filename, mode="rb") as f:
        for i, line in enumerate(f):
            ...
            if word in vocab:
                print("index is", i)
                print("origin index is", vocab[word])
                continue
...
def _expand_vocabulary(skip_thoughts_emb, skip_thoughts_vocab, word2vec):
    ...
    tf.logging.info("Selecting embeddings for %d shared words",
                    len(shared_words))
    ...
    tf.logging.info("Created expanded vocabulary of %d words",
                    len(combined_emb))
...
def main(unused_argv):
    ...
    skip_thoughts_emb = _load_skip_thoughts_embeddings(
        FLAGS.skip_thoughts_model)
    ...
    for v in embedding_map.values():
        values.append(v)
    print("the shape of values is ", len(values))
    print("the shape of values is ", len(values[0]))
    print("the type of values is ", type(values[0][0]))

    embeddings = np.array(values)
    embeddings_file = os.path.join(FLAGS.output_dir, "new_embeddings.npy")
